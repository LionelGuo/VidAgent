...
export const SYSTEM_KNOWLEDGE = {
  ...
  fieldsLine: "三个检索工具返回的每个视频都含 video_id/title/desc/publish_time/duration/duration_text/video_url/platform/author/view_count（duration 为秒数，duration_text 如 \"12:34\"，publish_time 为发布时间；view_count 依平台含义不同：bilibili/youtube 为播放量、xiaohongshu 为点赞数、douyin 热榜为热度值）。",
} as const;
