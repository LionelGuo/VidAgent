...
export const SYSTEM_KNOWLEDGE = {
  ...
  partsLine: "bilibili 存在分P视频：同一视频下含多个分P（每个分P有独立标题与时长，URL 以 p=N 指定）；检索结果中多P视频额外带 parts（分P清单：page/part/duration/duration_text）与 total_parts（分P总数；parts 少于 total_parts 时为截断，仅列前 50 个，仍可按 P 号指定任意分P）；总结入口对未指定分P的分P视频返回分P清单与分P条目，不直接总结（处理规则见行为规则段）。B站 UGC 合集/系列（跨多个独立视频）不属分P——各成员按独立视频对待，不展开。",
} as const;
