...
def _download_bili(url: str, file_name: str,
    ...
    opts = {
        ...
        # 1080p 封顶（与 YouTube 同理：抽帧只需 512×512，避免 4K 大文件）
    ...
    # 登录 Cookie 注入：部分视频/画质档无 cookie 下载会 412（CDN 风控）。
    # 转 Netscape cookiefile（覆盖 API 与 CDN 双域），避免 http_headers 方式的
    # cookie 泄漏警告（yt-dlp 弃用提示）与跨域风险。
...
def _write_bili_cookiefile(cookie: str) -> Path:
    """BILI_COOKIE 串 → 临时 Netscape cookiefile（API 与 CDN 双域）。

    用 tempfile 创建（不落 workspace，避免登录凭证残留），由调用方在
    ydl.download 结束后 unlink。
    """
    import tempfile

    lines = ["# Netscape HTTP Cookie File", "# generated by VidAgent"]
    for domain in (".bilibili.com", ".bilivideo.com"):
        for part in cookie.split(";"):
            ...
            if "=" not in part:
                continue
            k, v = part.split("=", 1)
            # domain TRUE(含子域) / FALSE(全路径) FALSE(非secure) 0(会话) name value
            lines.append(f"{domain}\tTRUE\t/\tFALSE\t0\t{k.strip()}\t{v.strip()}")
