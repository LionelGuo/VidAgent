...
class CDPBrowserManager:
    ...
    async def _test_cdp_connection(self, debug_port: int) -> bool:
        ...
        try:
            ...
            # [VidAgent patch] host 可配（CDP_DEBUG_HOST），见 PATCHES.md
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.settimeout(5)
                result = s.connect_ex(
                    (getattr(config, "CDP_DEBUG_HOST", "localhost"), debug_port)
                )
                if result == 0:
                    utils.logger.info(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.get(
                    f"http://{getattr(config, 'CDP_DEBUG_HOST', 'localhost')}:{debug_port}/json/version",
                    timeout=10,
                )
    ...
    async def _connect_via_cdp(self, playwright: Playwright):
        ...
        try:
            if config.CDP_CONNECT_EXISTING:
                ...
                # [VidAgent patch] host 可配（CDP_DEBUG_HOST），见 PATCHES.md
                ws_url = (
                    f"ws://{getattr(config, 'CDP_DEBUG_HOST', 'localhost')}:"
                    f"{self.debug_port}/devtools/browser"
                )
